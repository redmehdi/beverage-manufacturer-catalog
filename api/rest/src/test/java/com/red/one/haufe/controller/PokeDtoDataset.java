package com.red.one.haufe.controller;

import com.red.one.domain.entities.Beverage;

public final class PokeDtoDataset {
/**
    public static Beverage[] getSpecies() {
        return new Beverage[]{
                new Beverage(1L, "color1", "name1", getCharacters()[0]),
                new Beverage(2L, "color2", "name2", getCharacters()[1]),
                new Beverage(3L, "color3", "name3", getCharacters()[2]),
                new Beverage(4L, "color4", "name4", getCharacters()[3]),
                new Beverage(5L, "color5", "name5", getCharacters()[4])
        };
    }

    public static PokeCharacter[] getCharacters() {
        return new PokeCharacter[]{
                new PokeCharacter(1L, "name1", 3, 3, 3),
                new PokeCharacter(2L, "name2", 3, 3, 34),
                new PokeCharacter(3L, "name3", 3, 3, 5),
                new PokeCharacter(4L, "name4", 3, 3, 23),
                new PokeCharacter(5L, "name5", 3, 3, 88)
        };
    }*/

}
